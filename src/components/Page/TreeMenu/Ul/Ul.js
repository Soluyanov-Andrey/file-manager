...
const Ul = ({items, ext, visible}) => {
    ...
    return (
        <React.Fragment>
            <ul className="TreeMenu_ul">
                {items.map(n => (
                    <li key={n.adress} id={n.adress}>
                        <div className="TreeMenu_li_blok">
                            <img onClick={() => (ext(n.adress))} className="TreeMenu_li_side" src={image(n.presence_сhild, n.adress)}/>
                            <img className="TreeMenu_folder" height="20px" width="15px" src="image/TreeMenu/folder.png"/>{n.name}
                        </div>
